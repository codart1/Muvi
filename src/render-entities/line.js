...
export default class Line extends BaseRenderEntity {
  ...
  doCircularLine() {
    ...
    data.forEach((item, i) => {
      ...
      this.graphic.arcTo(this.cache[i - 1].x, this.cache[i - 1].y, x, y, 3);

      if (data.length - 1 === i) {
        this.graphic.arcTo(x, y, this.cache[0].x, this.cache[0].y, 3);
  ...
  doStraightLine() {
    ...
    data.forEach((item, i) => {
      ...
      this.graphic.arcTo(
        startX + lineWidth / data.length * (i - 1),
        middleY - data[i - 1],
        startX + lineWidth / data.length * i,
        middleY - item,
        3
      );
    });
